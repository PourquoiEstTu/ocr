...
it is. We created a python application
...
character recognition. We used machine learning
...
\textbf{Text recognition in natural scenes}. One work we have looked comes from \citealp{deCampos09}. Their work focuses on the recognition of text
in natural scenes. They created a photographs which contain either of English or Kannada text (we 
will focus on only the English here). The paper is primarily focused on how well machine learning 
models can model their dataset. They carry out experiments with different feature representations 
such as Shape Context and SIFT with nearest neighbour, support vector machines (SVM), and multiple 
kernel learning (MKL). Two of their conclusions were that even commercial OCR models were not 
suitable for this task and that shape based features such as Shape Context consistently 
outperformed appearance based features such as SIFT.

\textbf{Recognizing messy handwriting}. Another work we have looked at is from
\citealp{abdur_rahim_mia_abdullah_al_mamun_abdullah_al_sajid_aurunave_mollik_ruddra_2024}. Its 
focus is on recognizing handwritten prescriptions from doctors. It is motivated by handwriting 
being difficult to read and thus being possible to misinterpret by patients. It used the VGG16 
model to digitize the many hand-writing styles and drug names with good accuracy. The dataset was 
created by detecting images with words and then both manually discarding images with non-drug 
words on them along with manually labelling the images. 

\textbf{TextOCR dataset}. One more paper we read is from 
\citealp{DBLP:journals/corr/abs-2105-05486}. The main focus of this paper is the dataset they 
created so that OCR and scene text based reasoning research can be done better. The proposed 
dataset, TextOCR,
provides ``high quality and large quantity annotations'' 
(\citealp{DBLP:journals/corr/abs-2105-05486}, p. 1) on images. The former being easy to understand
and the latter coming from the dataset containing 28 000 images with 903 000 words in total. The 
paper goes on to explain how the dataset was annotated and how it compares against other datasets 
used for OCR model training. The researchers also developed a model called PixelM4C designed 
to ``text-based reasoning directly on the images unlike previous works which rely on pre-extracted 
OCR text and features'' (\citealp{DBLP:journals/corr/abs-2105-05486}, p. 2).

\textbf{TextVQA dataset}. The penultimate paper we looked at was 
\citealp{DBLP:journals/corr/abs-1904-08920}. It was about the new TextVQA dataset 
(VQA standing for Visual Question Answering)
they created and the model architecture they developed to solve problems in which 
looking at the text from an image is needed. The creation of TextVQA was motivated by previous
datasets for VQA not having many text-related questions and just generally being small in size.
The images used to make up the dataset are from the Open Images
v3 dataset. In particular, only images with text in them were used. To add the reasoning
data, annotators were asked to pose questions that required reading text from the 
image to answer. Annotators were further asked to come up with questions that differed from their
previous ones and were given prompts about some of the images in the dataset to check the 
quality of the data. The model architecture developed is called LoRRA (Look, Read, Reason,
and Answer). It has VQA and reading components to be able to answer the questions 
of the kind annotators were asked to produce. 

\textbf{Various text recognition tasks}. The final paper we looked at was \citealp{westerdijk2025improvingocrhistoricaltexts}. This paper
speaks about three different tasks: recognition of historical text, document layout analysis, and 
recognition of modern handwriting. The Kraken \citep{kiessling:hal-04936936} and TrOCR 
\citep{DBLP:journals/corr/abs-2109-10282} models
were used for the first task. The second task used the DeepLab
V3+ model \citep{DBLP:journals/corr/abs-1802-02611} for training. 
The last task used a Convolutional Recurrent Neural Network (CRNN) for feature extraction and then 
a multi-layer BiLSTM architecture for training. The first and third tasks were of particular
interest to us. A relevant conclusion of the first task was that careful data preparation and 
image processing was needed to make the model produce palatable results. The historical text 
data was very ``messy'' and small in size, so while the model did produce fine results 
(in part due to it being pretrained on a similar dataset), the training and prediction process
has many challenges. For the third task, the large conclusion was that model complexity and 
dataset size needs to be balanced. That is, good data augmentation on a small dataset can
lead to sizeable increases in model performance just as increasing the model complexity can.

The results from this final paper roughly guided us. While the rest of the 
paper's were very specific in their research and thus harder to apply for us,
this final one was more of a survey on text recognition techniques. We chose to  
use a convolutional network since it gave such good results. Also, this 
paper spoke to the importance of having good quality data from the outset and 
implementing good preprocessing so that the model can learn more from it. Due 
to this, we spent a lot of time on finding suitable preprocessing steps 
for our data, most of which will be detailed later.

\section{Dataset}
\hspace{\parindent}We chiefly used two datasets: 
\href{https://www.kaggle.com/datasets/harieh/ocr-dataset}{OCR-Dataset} and 
\href{https://www.kaggle.com/datasets/backalla/words-mnist}{Words MNIST}. 
...
our model on. It is licensed under CC0: Public Domain. The data is kept within 
folders with the folder names being the class names. For example, images 
with the uppercase ``A'' in them are stored with in the ``A\_U'' folder and images 
with the lowercase ``a'' in them are stored in the ``a\_L'' folder. The names 
of the images themselves are the same as the folder name with an underscore,
a number, and the image type appended to them. Using the first part of the 
example from above, an image stored within the ``A\_U'' folder may have name 
``A\_U\_$x$.png'' where $x$ is some number. One of our images is named 
``A\_U\_1003.png''.

There are a number of preprocessing steps done to the images 
from this dataset. Images are first converted to a specific type 
(\texttt{np.uint8})
...
pixels outside of it being removed from the image). The image is then scaled to 
have dimensions of 64x64, and finally, it is turned into a tensor that is 
accepted by PyTorch's \texttt{torch.nn.Conv2d} function. These tensors are 
saved to disk so that the preprocessing does not have to be repeated 
each time the model is trained. Before 
being fed to the model, a corresponding list of labels is generated from the 
provided labels and saved to disk for more convenient access similar to the 
features. These labels are converted from strings to numbers using 
scikit-learn's \texttt{LabelEncoder} as well. After this 
preprocessing, the data can be inputted to the model. The preprocessing as 
described so far assumes every single image contains a single character. So,
this preprocessing works for the images in this dataset; but, 
we are aiming to use this model trained only on such images on images with 
more than one character as well. Clearly, extra preprocessing is needed.
...
assortment of special characters/punctuation are in the words in the images. 
The labels are given in a dictionary in a \texttt{json} file. The images
are not contained within any folders nor have any structure; they are all 
located in one directory. The names of each image are some number with a dot 
and the file image file format appended to the number. For example, ``0.jpeg'' 
contained the word ``BE'' (case sensitivity is intended here). The images 
...
labelled using tesseract OCR and then manually checked after for errors. 
The images have variable sizes and need to be resized to be uniform. 
Be that as it may, our OCR model is built only to handle individual characters; 
so, before running 
the preprocessing from the previous paragraph that makes images suitable 
for being passed to the model, the characters in each 
word need to be segmented. We had two different attempts at this and will 
describe both. 

Our first, less successful segmentation, grey scaled 
the image, applied median blur to remove ``salt and pepper'' noise, 
applied inverse binary threshold using Otsu's method to automatically find the 
optimal threshold value/limit, and dilated the image 
...
up so that all characters looked roughly like rectangular blobs. We do this so 
that the contours algorithm finds a single contour (the contour of the rectangle
blob) for each letter, allowing us to detect each individual character. 
Finally, we used the dimensions of each contour to draw a bounding boxes them
and crop out the 
characters from the original image using them. Hence, we return an array of 
images containing each 
character. The large flaw of this approach is that if the dilation did not 
...
this occurred quite often with ``o''s and ``a''s. Another, albeit smaller
issue was the dilation making the columns occupied smaller. This would lead 
to the bounding boxes being ``tight'' on the characters and crop portions of them 
on the edge of the bounding boxes out of the returned images. This led to 
a small loss of information, but an obvious loss nonetheless.

The second, more successful segmentation function implements the Potential 
Segmentation Columns (PSC) method from \citealp{PSC}.
...
Suen's method to thin the characters. We chose not to use blurring with this 
method as images were more likely to lose line segments after thinning. 
For example, we had in image which contained ``you$\backslash$'':
\begin{center}
  \includegraphics[scale=0.4]{original-you.png}
\end{center}
With no blur applied, the thresholded and thinned image looked like this:
\begin{center}
  \includegraphics[scale=0.4]{thin-you.png}
\end{center}
With median blur applied before thresholding and thinning, the image looked 
like this:
\begin{center}
  \includegraphics[scale=0.4]{broken-you.png}
\end{center}
This decreased the accuracy of the column scanning. That is because
after thinning, all columns of pixels 
...
columns and the individual images are returned. Now that this has been described,
it is easy to see that the image using median blur will generate PSCs in the 
middle of all three letters; there are clear gaps within the three letters 
that will marked as PSCs. This results in very poor accuracy for the segmentation,
so median blur was dropped. This method worked much better
than the first one and is near guaranteed to work when there are clear vertical
gaps, even very small ones, between
characters. When there aren't any such gaps, it still performs well and better 
than the first method, but clearly not as good. This method was chosen for 
character segmentation due to its consistency (its effective guarantee to 
correctly segment characters when vertical gaps exist) and better quality.

\section{Features and Inputs}

\hspace{\parindent}The inputs to the current model are 64x64 dimension 
images. In the final version of our model, no feature engineering, 
...
a convolutional neural network (CNN). Our datasets were diverse in the fonts 
they used, so there are a good few words that are slanted, have curving and
curling line segments. Due to this diversity, we decided not do feature 
augmentation. Another reason is that we expect users to input images in the 
correct orientation. Although it is very much possible to not do as such 
(we are very much aware users often do not follow the expectations of developers),
we wanted to solely focus on increasing accuracy for ``normal'' text.

Before we finalized our model, we used a different 
set of features. Originally, we generated Histogram of Oriented Gradients 
(HOG) features to feed our model. This is an old and established feature 
engineering technique that can be used for object detection. Due to this, we 
chose to use it for our early 
...
When we were training on our personal devices, we tended to go with lower 
cell sizes and thus lower feature counts due to Random Access Memory (RAM) 
constraints. When training on one of McMaster's GPU clusters, we increased 
the cell size and thus the feature count accordingly.
...
\begin{center}
  \includegraphics[scale=0.2]{mistral-benchmarks.png}
\end{center}
% \begin{figure}[htbp]
%     \centering
...
blocks to boost our accuracy. We use this model in a python file that takes a input
parameter of the path to where a image is stored, and outputs the predicted text obtained 
from running out CNN model on the segmented images. 
...
\section{Evaluation}
The Main evaluation metrics that we tracked to measure how well our models were doing were Character 
Classification Accuracy and Word Recognition Accuracy. 

\subsection*{Character Classification Accuracy}
We evaluate the character classifcation accruacy by calculating the accuracy, precision, 
recall, and F1-Score after training our models, and comparing it to eachother and a 
basic "majority class" baseline. (predicting the label for a image based on their 
appearance in our dataset). 

Examining our HOG model, we can see the accuracy, precision, recall, F1-Score, and
support values obtained after training in \autoref{tab:old_results}. Looking at  \autoref{tab:old_results} we can see that this model
achieves an overall accuracy of 67\% on the test set of 682 samples. The macro-averaged metrics show 
precision of 68\% and the weighted averaged is 70\%. This represents a strong improvement over 
the baseline and demonstrates that the model has learned meaningful patterns from
the HOG features. This beats the baseline on our dataset which would have a 1.6\% accuracy, as the data the HOG model
is tested on contains 62 classes, each with 52 images each, giving each class a 1/62=1.6\% chance of being chosen.

Examining our CNN model, the accuracy, precision, recall, F1-Score, and support values obtained after training
can be viewed in \autoref{tab:new_results}. Looking at \autoref{tab:new_results} we can see that this model achieves a overall accuracy of 90\% on
the test set of 21023 samples. The macro-averaged metrics show 
precision of 91\% and the weighted averaged is 91\%. This easily beats the baseline on this dataset which is 1.6\% 
accuracy, as the dataset contians 62 classes, each with approximately 3475 images each, giving each class a
approximate probablty of 1/62=1.6\% chance of being chosen.


\subsection*{Word Recognition Accuracy}

Using the MNIST dataset containing 10K printed text words, we chose 6001 images from it to test our model on. 
These images were chosen from which ones resulted in a "valid word segmentation". We determined this by running 
our word segmentation code upon it, and if the number of segmented images returns from it matched the amount of 
letters in the images label, it was a valid word segmentation. This means that our word segmentation algorithm 
failed to accurately segment 3999 of the images.
...
Looking the confusion matrix generated for validation set of HOG model, seen in \autoref{fig:confusion_matrix}. \autoref{fig:confusion_matrix} shows both the classes 
...
The confusion matrix generated for the CNN model can be seen \autoref{fig:CNN_confusion_matrix}. \autoref{fig:CNN_confusion_matrix} shows a similar pattern as the HOG model, 
...
increase the resolution of the input images. Our current $64\times64$ inputs may be too low to capture the subtle 
stroke differences between similar characters such as O vs 0 or C vs c. Increasing this to $96\times96$ or higher 
...
Dhruv and Arin developed the current iteration of the Convolutional neural network model, including creating 
HOG linear neural network, the class for the model, writing the training code, measuring the accuracy of the model, and 
creating a confusion matrix for it. Also worked on preprocessing for the CNN model, training for both HOG and CNN. Worked on the creation of final script that does the classifying. Lastly, implemented additional preprocessing code to make the data 
suitable for PyTorch models along with code to ensure that if a GPU is present, it will be 
used in the training process. 

Roshaan contributed to finding the datasets we're using as well as implementing the 
feature generation and general preprocessing code to input data into the scikit-learn SVMS and 
PyTorch neural networks. He also attempted to implement
multiclass SVMs and in tandem with this, he implemented further preprocessing code to make the 
data suitable for inputting into the SVM. Lastly, he did troubleshooting for GPU clusters.
...
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[width=0.5\textwidth]{cnn_loss_curve.png}
%     \caption{HOG Confusion Matrix}
...
Z & 0.69 & 0.75 & 0.72 & 12 & & & & & \\
\hline
\multicolumn{10}{l}{\textbf{Accuracy: 67\%} \qquad \textbf{Macro Avg Prec:} 0.68 \qquad \textbf{Weighted Avg Prec:} 0.70} \\
\hline
\end{tabular}
}
...
\hline
\multicolumn{10}{l}{\textbf{Accuracy: 90\%} \qquad \textbf{Macro Avg Prec:} 0.91 \qquad \textbf{Weighted Avg Prec:} 0.91} \\
\hline
\end{tabular}
}
